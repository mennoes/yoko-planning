...
// DEMO-VARIANT van lib/todoProjectSeed.ts — zelfde logica, maar leest de
// verzonnen demo-borden (lib/demoFixtures.ts) i.p.v. de echte 5 borden.
...
import { buildDemoBoards } from './demoFixtures'
...
function loadRemovedProjectKeys(): Set<string> {
  if (typeof window === 'undefined') return new Set()
  try {
    const raw = window.localStorage.getItem('yoko-demo-todos-removed-projects')
