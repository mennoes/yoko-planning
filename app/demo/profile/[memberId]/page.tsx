...
// DEMO-VARIANT van app/profile/[memberId]/page.tsx — zelfde component,
// maar elke Supabase-lees/schrijf-actie vervangen door lokale
// (localStorage-only) demo-opslag. Zie app/demo/planning/page.tsx voor
// hetzelfde patroon.
...
import { loadDemoExtendedProfile, saveDemoExtendedProfile } from '@/lib/demoProfileStore'
...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    setData(loadDemoExtendedProfile(memberId) as ExtendedProfile | null)
    setLoaded(true)
  }, [memberId])

  async function persistField(patch: Partial<ExtendedProfile>) {
    if (!isMe) return
    setData(saveDemoExtendedProfile(memberId, patch) as ExtendedProfile)
  }
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: isMobile ? '24px 16px 80px' : '40px 36px 100px' }}>
          ...
          <ClickableAvatar
            ...
            onPicked={(dataUrl) => {
              if (myProfile) {
                ...
                setData(saveDemoExtendedProfile(memberId, { photo: dataUrl }) as ExtendedProfile)
              }
            }} />
      ...
      {/* DEMO: geen wachtwoord-sectie — geen echt account om te beveiligen. */}
