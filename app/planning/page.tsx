...
function DraggableBar({ project, memberId, team, left, width, colW, small, laneH, stackH, laneIdx, scaleByHours, onDragMove, onDragEnd, onClick, onReassign }: {
  ...
  laneH?: number
  // Wrapper-stack-hoogte (alle lanes samen). Gebruikt voor 't cappen
  // van lane-positionering zodat 'n hoge laneIdx niet voorbij 't einde
  // van de stapel-wrapper schuift. Default = laneH.
  stackH?: number
  ...
  // 15% baseline + 85% schaal. Met de ruimere lane-hoogte (40px week)
  // levert dit goed leesbare bars: 1u/dag = 26% lane (~10px), 4u/dag =
  // 58% (~22px), 8u/dag = 100% (~38px). 6px floor.
  const scaledRatio = 0.15 + 0.85 * ratio
...
function TimelineBars({ memberId, projects, team, cols, colW, zoom, hideMeetings, rowScale, onDragMove, onDragEnd, onBarClick, onReassign }: {
  ...
  // PROJECT_LANE_H bepaalt 1 lane-hoogte (= max bar-hoogte voor 8u/dag).
  // Trade-off: groter = grotere, beter leesbare bars; kleiner = compactere
  // wrapper. 40px in week-zoom past 'n 4u/dag-bar comfortabel (~24px),
  // en 'n full 8u-bar is duidelijk maar overdrijft niet.
  const PROJECT_LANE_H = Math.round((zoom === 'maand' ? 28 : zoom === 'week' ? 40 : (BAR_H + BAR_GAP)) * RS)
...
export default function PlanningPage() {
  ...
  useEffect(() => {
    ...
    function onPushFail(e: Event) {
      ...
      const msg = ce.detail?.message ?? 'Opslaan mislukt — wijziging blijft alleen lokaal staan.'
      showToast(msg, 8000)
    }
    window.addEventListener('focus', onFocus)
    document.addEventListener('visibilitychange', onVisibility)
    window.addEventListener('yoko-push-failed', onPushFail)
    ...
    return () => {
      ...
      window.removeEventListener('yoko-push-failed', onPushFail)
