...
export default function TeamAdminPage() {
  ...
  async function toggleInactive(id: string) {
    const current = members.find(m => m.id === id)
    if (!current) return
    if (!supabase) { alert('Supabase niet geconfigureerd.'); return }
    const { data } = await supabase.auth.getSession()
    const token = data.session?.access_token
    if (!token) { alert('Je bent niet ingelogd — log opnieuw in en probeer het dan.'); return }
    const response = await fetch('/api/team/status', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, inactive: !current.inactive }),
    })
    const result = await response.json() as { ok: boolean; error?: string }
    if (!response.ok || !result.ok) {
      alert(`Wijzigen mislukt: ${result.error ?? response.statusText}`)
      return
    }
    await refresh()
  }
